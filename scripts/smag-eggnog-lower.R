...
pbmclapply(smags_lower, function(X) {
  fname <-
    file.path(
      "results/lowestOG/01_Results_EggNog_annotationV1-filt",
      paste0(X, emapper_files_pattern_filt)
    )
  emapper_results_lower %>%
    filter(sMAG == X) %>%
